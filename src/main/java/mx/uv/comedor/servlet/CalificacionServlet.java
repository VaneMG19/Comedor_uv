...
@WebServlet(urlPatterns = {"/calificaciones","/calificaciones/crear",
        "/calificaciones/responder","/calificaciones/aprobar"})
public class CalificacionServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if (idParam == null) {
                ...
                req.getRequestDispatcher("/WEB-INF/vistas/admin/calificaciones.jsp").forward(req, resp);
            } else {
                Long idPlatillo = Long.parseLong(idParam);
                List<Calificacion> lista = califDAO.listarPorPlatillo(idPlatillo);
                EstadisticaPlatillo est  = califDAO.obtenerEstadistica(idPlatillo);
                req.setAttribute("calificaciones", lista);
                req.setAttribute("estadistica", est);
                req.setAttribute("idPlatillo", idPlatillo);
                req.getRequestDispatcher("/WEB-INF/vistas/calificaciones-platillo.jsp").forward(req, resp);
            }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            switch (req.getServletPath()) {
                case "/calificaciones/crear":     crear(req, resp);     break;
                case "/calificaciones/responder": responder(req, resp); break;
                case "/calificaciones/aprobar":   aprobar(req, resp);   break;
            }
        } catch (SQLException e) { manejarError(req, resp, e); }
    }
    ...
    private void crear(HttpServletRequest req, HttpServletResponse resp)
        ...
        Calificacion c = new Calificacion(usuario.getIdUsuario(), idPlatillo, idPedido, puntuacion, comentario);
        califDAO.insertar(c);
        resp.sendRedirect(req.getContextPath() + "/pedido/historial?exito=calificado");
    }
    ...
    private void responder(HttpServletRequest req, HttpServletResponse resp)
        ...
        RespuestaCalificacion r = new RespuestaCalificacion(idCalif, admin.getIdAdmin(), respuesta);
        califDAO.insertarRespuesta(r);
        resp.sendRedirect(req.getContextPath() + "/calificaciones?exito=respondida");
    }
